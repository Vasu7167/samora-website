...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form?.addEventListener('submit', e => {
    ...
    // Remember the real label. It was hardcoded back to 'Request a Reality
    // Audit', which is not what the Contact page button says, so a successful
    // submit silently relabelled the button.
    ...
    // Posts to our own /api/contact, which STORES the enquiry first and then
    // emails a notification. This used to post to
    // https://formspree.io/f/YOUR_FORM_ID, a placeholder that was never
    // filled in, so every enquiry ever submitted 404'd and was lost.
    fetch('/api/contact', {
      method: 'POST',
    ...
    .then(async res => {
      ...
      if (res.ok && data.ok) {
        ...
        // Say what actually went wrong. Someone who is told their email was
        // rejected can fix it; someone shown a bare "Try again" leaves.
        err(data.error || 'Something went wrong. Please email sam@samoraglobal.com.');
      }
    })
    .catch(() => {
      b.textContent = 'Try again';
      b.disabled = false;
      err('Could not reach the server. Please email sam@samoraglobal.com.');
    });
  });
